...
public function actualizarPerfil($id, $nombre, $apellido, $mail, $centro, $sector, $clase, $idioma, $nueva_password = '') {
    $db = new AccesoBD();
    $conn = $db->conexion;
    ...
    if (!empty($nueva_password)) {
        ...
        $sql = "UPDATE user SET nombre = ?, apellido = ?, mail = ?, centro = ?, sector = ?, clase = ?, idioma = ?, password = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssiissi", $nombre, $apellido, $mail, $centro, $sector, $clase, $idioma, $password_hash, $id);
    } else {
        $sql = "UPDATE user SET nombre = ?, apellido = ?, mail = ?, centro = ?, sector = ?, clase = ?, idioma = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssiiisi", $nombre, $apellido, $mail, $centro, $sector, $clase, $idioma, $id);
    }
