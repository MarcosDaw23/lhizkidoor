...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nombreEvento && ($clases || $sectores)) {
        $profesor_id = $_SESSION['user']['id'];
        $db = new AccesoBD_Profesor();

        $evento_id = $db->crearEvento($nombreEvento, $profesor_id, $num_preguntas, $clases, $sectores);

        if ($evento_id) {
            $usuarios = $db->obtenerUsuariosEvento($clases, $sectores);
            ...
            foreach ($usuarios as $u) {
                ...
                $link = "http://localhost/1semestre/lhizkidoor/usuario/index.php?section=partidaEvento&evento=$evento_id";
                ...
                try {
                    ...
                    $mailer->Body = "
                        <p>Hola <b>{$nombre}</b>,</p>
                        <p>Tu profesor ha creado un nuevo evento en <b>LHizkiDoor</b> para ti.</p>
                        <p>Este evento contiene <b>{$num_preguntas}</b> preguntas y está disponible desde este enlace:</p>
                        <p><a href='$link'>$link</a></p>
                        <br>
                        <p>Solo podrás jugar este evento si has recibido esta invitación.</p>
                        <p>¡Mucha suerte!</p>
                    ";
